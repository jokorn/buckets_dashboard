...
dateRangeInput2 <- function(inputId, label, start = NULL, end = NULL,
  ...
  start <- shiny:::dateYMD(start, "start")
  end   <- shiny:::dateYMD(end, "end")
  ...
  datePickerDependency <- function(theme) {
    list(
      htmltools::htmlDependency(
        ...
        version = shiny:::version_bs_date_picker,
...
date_from <- max(today2() %>% floor_date("year"), dates_available[1])
date_to <- today2()
